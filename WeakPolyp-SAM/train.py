...
def train(Dataset, Network, args):
    ...
    for n, p in sam.mask_decoder.named_parameters():
        p.requires_grad = False
    for n, p in sam.image_encoder.named_parameters():
        p.requires_grad = False
    for n, p in sam.image_encoder.blocks[8].named_parameters():
        p.requires_grad = True
    for n, p in sam.image_encoder.blocks[9].named_parameters():
        p.requires_grad = True
    for n, p in sam.image_encoder.blocks[10].named_parameters():
        p.requires_grad = True
    for n, p in sam.image_encoder.blocks[11].named_parameters():
        p.requires_grad = True
    for n, p in sam.mask_decoder.output_upscaling.named_parameters():
        p.requires_grad = True
    ...
    for epoch in range(cfg.epoch):
        ...
        while image is not None:
            ...
            for i, pack in enumerate(zip(img_sam, sam_label, pred_label)):
                ...
                box_s = get_boxes_from_mask(scribble_box.detach().cpu())
                box_p = get_boxes_from_mask(pred_box.detach().cpu())
                box = Union_box(box_s, box_p)
                dict = {'image': img, 'original_size': (320, 320), 'boxes': box.cuda()}
                list_s.append(dict)
            out = sam(list_s, multimask_output=False)
            sam_o = []
            indicator = []
            for i in range(len(out)):
                mask, flag = Filter(out[i]['masks'], sam_label[i])
                sam_o.append(mask)
                indicator.append(flag)
            label_sam = torch.cat(sam_o, dim=0)
            indicator_t = torch.IntTensor(indicator)
            loss = loss2 + loss_ssc + loss3 * 0.6 + loss4 * 0.4 + loss5 * 0.2
            if torch.all(indicator_t == 0).item() == False:
                loss_sam2 = structure_loss(out2[:, 1:2][indicator_t == 1], label_sam[indicator_t == 1].float()).cuda()
                loss_sam3 = structure_loss(out3[:, 1:2][indicator_t == 1], label_sam[indicator_t == 1].float()).cuda()
                loss_sam4 = structure_loss(out4[:, 1:2][indicator_t == 1], label_sam[indicator_t == 1].float()).cuda()
                loss_sam5 = structure_loss(out5[:, 1:2][indicator_t == 1], label_sam[indicator_t == 1].float()).cuda()
                loss_sam = loss_sam2 + loss_sam3 * 0.6 + loss_sam4 * 0.4 + loss_sam5 * 0.2
                loss = loss + loss_sam * l
            ...
            optimizer.step()
            if batch_idx % 10 == 0:
                msg = '%s| %s | step:%d/%d/%d | lr=%.6f | loss=%.6f | loss2=%.6f  | loss_ssc =%.6f| loss3=%.6f  | loss4=%.6f | loss5 =%.6f |  losssam2=%.6f  | loss_sam3 =%.6f| losssam4=%.6f ' % (
                    SAVE_PATH, datetime.datetime.now(), global_step, epoch + 1, cfg.epoch,
                    optimizer.param_groups[1]['lr'],
                    loss.item(), loss2.item(),
                    loss_ssc.item(), loss3.item(), loss4.item(), loss5.item(), loss_sam2.item(), loss_sam3.item(),
                    loss_sam4.item())
                print(msg)
                logger.info(msg)
            image, mask, img_sam, size, names = prefetcher.next()
...
if __name__ == '__main__':
    ...
    args.checkpoint = './lib/sam_vit_b_01ec64.pth'
